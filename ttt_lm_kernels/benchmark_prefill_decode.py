...
if is_mamba:
    ...
    ttt_config = TTTConfig(**TTT_STANDARD_CONFIGS['1b'], vocab_size=16384)
...
if is_mamba or is_ttt:
    model.eval()
    logger.info(f"Number of parameters: {sum(p.numel() for p in model.parameters())}\n")
    input_ids = torch.randint(1, 16384, (args.batch, args.prompt_len), dtype=torch.long, device=device)
    ...
    input_ids = [
        torch.randint(low=1, high=16384, size=(args.prompt_len,)).tolist() for _ in range(args.batch)
    ]
